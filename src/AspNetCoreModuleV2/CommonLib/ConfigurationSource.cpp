...
std::vector<std::pair<std::wstring, std::wstring>> WebConfigConfigurationSection::GetKeyValuePairs(const std::wstring& name)
{
    auto hr = [&, this]() -> HRESULT
    {
        ...
        RETURN_IF_FAILED(element->get_Collection(&elementCollection));

        RETURN_IF_FAILED(findElementResult = FindFirstElement(elementCollection, &index, &collectionEntry));

        while (findElementResult != S_FALSE)
        {
            CComBSTR strHandlerName;
            RETURN_IF_FAILED(GetElementStringProperty(collectionEntry, L"name", &strHandlerName.m_str));

            CComBSTR strHandlerValue;
            RETURN_IF_FAILED(GetElementStringProperty(collectionEntry, L"value", &strHandlerName.m_str));

            collectionEntry.Release();

            RETURN_IF_FAILED(findElementResult = FindNextElement(elementCollection, &index, &collectionEntry));
        }
        return S_OK;
    }();
